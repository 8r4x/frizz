...
execFileSync("sqlite3", [db, `INSERT OR REPLACE INTO session (slug, session_id, tmux_name, spawned_at, title, backend, model, effort, permission_mode)
  VALUES ('${SLUG}', '${SESSION}', 'fray-${SLUG}', '${now()}', 'Scroll jitter', 'claude', 'opus', 'high', 'default')`])
...
try {
  ...
  check("the probe really travelled back through the transcript (not passing vacuously)",
    result.frames >= steps * 0.8 && result.startedAt - result.endedAt > 2000,
    `frames=${result.frames} travelled=${result.startedAt - result.endedAt}px`)
