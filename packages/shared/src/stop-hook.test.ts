// The stop hook's two pure pieces: the sentinel parser that CLOSES the loop, and the trailer
...
import { STOP_HOOK_SENTINEL, saysAllDone, stopHookMessage } from "./index.ts"
...
// The expensive failure is the FALSE POSITIVE: it silences a stop hook the operator armed, with
...
test("stopHookMessage leads with the operator's words and teaches the sentinel it parses", () => {
