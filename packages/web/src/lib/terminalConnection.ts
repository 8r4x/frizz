...
// The server reports a sign-in CLI that has exited as a clean websocket close whose reason is
// `pty exit <code>` — a wire token kept from when the CLI ran on a pty. Everything else (server
// replacement, network loss, laptop sleep, proxy reset) is transport failure and should reconnect
// without throwing away the xterm buffer.
