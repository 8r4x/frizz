...
import { probeDevFrayBuild, resetDevFrayBuildProbe } from "./devBuild.ts"
...
test("every caller shares ONE supervisor request, and later callers get the cached answer", async () => {
  ...
  const concurrent = Promise.all([probeDevFrayBuild(), probeDevFrayBuild(), probeDevFrayBuild()])
  release()
  ...
  assert.equal(await probeDevFrayBuild(), true)
...
test("an unreachable supervisor reads as not-dev, and is asked again next time", async () => {
  ...
  assert.equal(await probeDevFrayBuild(), false, "never show a dev-only verb on no evidence")
  assert.equal(calls(), 1)

  down = false
  assert.equal(await probeDevFrayBuild(), true, "the next mount re-probes rather than trusting a failure")
  assert.equal(calls(), 2)
})

test("a published Fray, which omits the field entirely, is cached as not a dev build", async () => {
  resetDevFrayBuildProbe()
  const calls = stubFetch(async () => json({ protocol: 1, state: "ready", updateRestart: true }))

  assert.equal(await probeDevFrayBuild(), false)
  assert.equal(await probeDevFrayBuild(), false)
