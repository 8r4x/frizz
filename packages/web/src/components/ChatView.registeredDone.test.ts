...
test("the card is the last rung of the ladder, after the resting card, in both paths", () => {
  ...
  assert.equal(rungs.length, 2, "the rung must close the chain in the plain and the virtualized ladder")
...
test("both gates include it, so the slot opens exactly when the rung renders", () => {
  assert.match(chat, /showsRestingCard\(thread\) \|\| registeredDone\) && \(/, "the plain path's spacer gate")
