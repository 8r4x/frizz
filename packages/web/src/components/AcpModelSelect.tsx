...
import { acpModelOptions } from "../lib/acpModelOptions.ts"
import { OPAQUE_PORTAL_SURFACE_Z } from "../lib/overlaySurface.ts"
import { Select } from "./ui/Select.tsx"

// THE MODEL DROPDOWN BESIDE AN ACP AGENT'S PROFILE PILL (maintainer 2026-09-16: "it should be a
// dropdown picker, not the grid"). The grid picks the AGENT — one row per installed agent, no effort
// axis — and this control picks the model inside it. The list is the agent's own: the server opens a
// throwaway session and reads the `model` config option (rpc.acpAgentModels, cached ten minutes), so
// an agent that is not signed in lists nothing and the control degrades to the agent's default with
// the reason in its tooltip rather than failing the composer.
//
...
export function AcpModelSelect({
  ...
  const options = acpModelOptions(models.data, modelId)
