...
test("a reload paints the font, the project rail and the sidebar column in their final state on the first frame", {
  ...
  try {
    ...
    // CONTROL: no mirrors and the rail on the server — the same sampler must see the rail arriving
    // late and the workpane moving. (The font half of this control went with the mono setting.)
    await set({ projectRail: true })
