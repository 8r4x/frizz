...
import { insideFence, PermissionMode, saysAwaiting } from "@fray-ui/shared"
...
export function applyRecord(state: TailState, rec: Record): void {
  ...
  if (type === "permission-mode") {
    ...
    if (raw !== undefined) {
      ...
      state.lastAssistantAwaiting = saysAwaiting(raw)
    ...
    state.lastAssistantAwaiting = false
...
function applyFinalText(state: FoldState, text: string): void {
  ...
  state.lastAssistantAwaiting = saysAwaiting(text)
