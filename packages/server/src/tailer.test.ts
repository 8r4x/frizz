...
// A MANUAL `/compact` is a local command, not a model turn, and the transcript it leaves never ends one.
// The sequence below is a real broker session on Claude Code 2.1.282 (2026-09-25), trimmed to the fields
// the fold reads: the thread rests, the human sends `/compact`, and the CLI writes the boundary, the
// carry-over summary, the command's own envelope, and its `<local-command-stdout>` receipt — then only
// sidecars. No assistant record follows, so every one of those user records read as "the model is about
// to respond" and the board showed the thread running until the next real message (3+ minutes live).
test("computeTurn: a manual /compact comes to rest once its local-command receipt lands", () => {
  const s = newTailState("t", "sid", "/x")
  const sidecar = { userType: "external", entrypoint: "sdk-ts", sessionId: "sid", version: "2.1.282" }
  const records: Record<string, unknown>[] = [
    { ...sidecar, type: "assistant", timestamp: "2026-09-25T20:10:32.106Z", message: { model: "claude-opus-5-5", role: "assistant", stop_reason: "end_turn", content: [{ type: "text", text: "ok\n\n```done\n- **Replied** with `ok`.\n```" }] } },
    { type: "system", subtype: "stop_hook_summary", timestamp: "2026-09-25T20:10:32.152Z" },
    { type: "queue-operation", operation: "enqueue", timestamp: "2026-09-25T20:14:45.455Z", sessionId: "sid", content: "/compact" },
    { type: "queue-operation", operation: "dequeue", timestamp: "2026-09-25T20:14:45.455Z", sessionId: "sid" },
    { type: "last-prompt", lastPrompt: "Reply with the single word: ok", sessionId: "sid" },
    { type: "ai-title", aiTitle: "Acknowledge system setup", sessionId: "sid" },
    { type: "atis-latch", atis: "", sessionId: "sid" },
    { ...sidecar, type: "system", subtype: "compact_boundary", content: "Conversation compacted", isMeta: false, timestamp: "2026-09-25T20:14:54.580Z", level: "info", compactMetadata: { trigger: "manual", preTokens: 48878, durationMs: 9090, postTokens: 4121 } },
    { ...sidecar, type: "user", isCompactSummary: true, isVisibleInTranscriptOnly: true, timestamp: "2026-09-25T20:14:54.580Z", message: { role: "user", content: "This session is being continued from a previous conversation that ran out of context. The summary below covers the earlier portion of the conversation.\n\nSummary:\n1. Primary Request and Intent: …" } },
    { ...sidecar, type: "user", isMeta: true, timestamp: "2026-09-25T20:14:45.490Z", message: { role: "user", content: "<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond to these messages or otherwise consider them in your response unless the user explicitly asks you to.</local-command-caveat>" } },
    { ...sidecar, type: "user", timestamp: "2026-09-25T20:14:45.490Z", message: { role: "user", content: "<command-name>/compact</command-name>\n            <command-message>compact</command-message>\n            <command-args></command-args>" } },
    { ...sidecar, type: "user", timestamp: "2026-09-25T20:14:55.057Z", message: { role: "user", content: "<local-command-stdout>Compacted PreCompact [node \"${CLAUDE_PLUGIN_ROOT}/hooks/scratchpad.mjs\" --mode=precompact] completed successfully</local-command-stdout>" } },
    { ...sidecar, type: "attachment", timestamp: "2026-09-25T20:14:54.586Z", attachment: { type: "environment" } },
    { ...sidecar, type: "attachment", timestamp: "2026-09-25T20:14:54.586Z", attachment: { type: "hook_success", hookName: "SessionStart:compact", hookEvent: "SessionStart" } },
    { ...sidecar, type: "attachment", timestamp: "2026-09-25T20:14:54.586Z", attachment: { type: "hook_additional_context", content: ["⟦scratch directory⟧ Context was just compacted or resumed…"] } },
    { type: "last-prompt", lastPrompt: "Reply with the single word: ok", sessionId: "sid" },
    { type: "ai-title", aiTitle: "Acknowledge system setup", sessionId: "sid" },
    { type: "atis-latch", atis: "", sessionId: "sid" },
  ]
  for (const rec of records) applyRecord(s, rec as Parameters<typeof applyRecord>[1])
  const receiptAt = Date.parse("2026-09-25T20:14:55.057Z")
  assert.equal(s.lastCompactionAt, "2026-09-25T20:14:54.580Z", "the compaction clock still starts")
  // The same 5s silence an interrupt receipt gets, for the same reason: a message queued behind the
  // command lands milliseconds later and must not flash a rest first.
  assert.equal(computeTurn(s, receiptAt + 1_000), "in-flight")
  assert.equal(computeTurn(s, receiptAt + 6_000), "idle", "the command is over and nothing followed it")
  assert.equal(computeTurn(s, receiptAt + 180_000), "idle")
  // …and the next real message opens a turn as usual.
  applyRecord(s, { type: "user", timestamp: "2026-09-25T20:18:00.000Z", message: { content: "and now?" } })
  assert.equal(computeTurn(s, receiptAt + 600_000), "in-flight")
})

// The CONTROL: an AUTOMATIC compaction runs inside a live turn — the model resumes from the summary and
// writes the turn's end itself — so the summary alone must keep reading as in-flight however long the
// model takes. Only the local command's receipt says the turn is over.
test("computeTurn: an automatic compaction's carry-over summary keeps the turn in flight", () => {
  const s = newTailState("t", "sid", "/x")
  applyRecord(s, { type: "assistant", timestamp: "2026-09-25T20:00:00.000Z", message: { stop_reason: "tool_use", content: [{ type: "tool_use", id: "t1", name: "Bash", input: {} }] } })
  applyRecord(s, { type: "user", timestamp: "2026-09-25T20:00:01.000Z", message: { content: [{ type: "tool_result", tool_use_id: "t1", content: "done" }] } })
  applyRecord(s, { type: "system", subtype: "compact_boundary", timestamp: "2026-09-25T20:00:02.000Z", compactMetadata: { trigger: "auto" } } as Parameters<typeof applyRecord>[1])
  applyRecord(s, { type: "user", isCompactSummary: true, timestamp: "2026-09-25T20:00:02.000Z", message: { content: "This session is being continued from a previous conversation…" } })
  assert.equal(computeTurn(s, Date.parse("2026-09-25T20:05:00.000Z")), "in-flight")
  // A human message that merely QUOTES the tag is not the CLI's receipt.
  applyRecord(s, { type: "user", timestamp: "2026-09-25T20:05:01.000Z", message: { content: "why did it print <local-command-stdout>x</local-command-stdout>?" } })
  assert.equal(computeTurn(s, Date.parse("2026-09-25T20:10:00.000Z")), "in-flight")
})
...
// ---- THE FENCE IS RETIRED (2026-09-11): a free-form ```question is prose for a thread dispatched at or
// after QUESTION_FENCE_RETIRED_AT, and still an ask for one dispatched before it. ONE gate, in `get`:
// the fold keeps recording the fact about the text; the telemetry every consumer reads is what is gated.
const BEFORE_RETIREMENT = new Date(Date.parse(QUESTION_FENCE_RETIRED_AT) - 60_000).toISOString()
...
test("hasQuestionBlock: the empty placement marker (```question qst_…) has no body and never counts", () => {
  ...
  // …and the fold agrees, for a legacy-shaped state where the flag is not gated.
  const s = newTailState("t", "s", "/x")
  applyRecord(s, { type: "assistant", timestamp: "2026-07-01T00:00:02.000Z", message: { stop_reason: "end_turn", content: [{ type: "text", text: "Placed:\n\n```question qst_ab12cd34\n```" }] } })
  assert.equal(s.lastAssistantHasQuestion, false)
})

test("tailer: a ```question fence queues a LEGACY thread and is prose for one dispatched after the retirement", () => {
  const FENCE = "Verdict: duplicate.\n\n```question\nHow should PRD-8263 be dispatched?\n\n- A. Fix now\n- B. File only\n```"
  const ASKED = JSON.stringify({ type: "assistant", timestamp: "2026-07-01T00:00:02.000Z", message: { stop_reason: "end_turn", content: [{ type: "text", text: FENCE }] } })
  for (const [spawnedAt, live] of [[BEFORE_RETIREMENT, true], [AFTER_RETIREMENT, false]] as const) {
    const h = harness()
    h.storage.upsertSession(row({ spawned_at: spawnedAt }))
    fixture(h.logDir, "sid", [IN_FLIGHT, ASKED])
    h.clock.ms = Date.parse("2026-07-01T00:00:10.000Z")
    const t = makeTailer(h)
    t.tick()
    const tele = t.get("t")
    assert.equal(tele?.turn, "idle")
    assert.equal(tele?.pendingQuestion, live, `spawned_at=${spawnedAt}: pendingQuestion`)
    // The consumers inherit it, in the board's own composition (degrade, then the queue rule):
    // a running legacy thread is degraded to turn-idle by its fence and queued as a HARD member; a
    // running new-contract thread with the same text stays running and is not queued at all.
    const runtime = degradeIfAwaitingAnswer("running", tele?.pendingQuestion)
    assert.equal(runtime, live ? "turn-idle" : "running", `spawned_at=${spawnedAt}: degrade`)
    assert.equal(deriveNeedsYou(row({ spawned_at: spawnedAt }), tele, runtime), live, `spawned_at=${spawnedAt}: queued`)
    // And at rest the fence is no longer a hard member: a live sub-agent excuses a bare rest from the
    // queue, which a legacy fence outranks and a new-contract fence — being prose — does not. (A bare
    // rest with no live work still queues either way; that is the rest, not the fence.)
    const withChild = { ...tele!, subAgents: [{ label: "c", startedAt: "2026-07-01T00:00:01.000Z", state: "running" as const, id: "a1" }] }
    assert.equal(deriveNeedsYou(row({ spawned_at: spawnedAt, rested_at: "2026-07-01T00:00:02.000Z" }), withChild, "turn-idle"), live, `spawned_at=${spawnedAt}: hard member`)
  }
})

test("tailer: the fence gate reads the LIVE row, so a respawn that bumps spawned_at past the retirement retires the fence", () => {
  const ASKED = JSON.stringify({ type: "assistant", timestamp: "2026-07-01T00:00:02.000Z", message: { stop_reason: "end_turn", content: [{ type: "text", text: "```question\nWhich?\n\n- A. x\n```" }] } })
  const h = harness()
  h.storage.upsertSession(row({ spawned_at: BEFORE_RETIREMENT }))
  fixture(h.logDir, "sid", [IN_FLIGHT, ASKED])
  h.clock.ms = Date.parse("2026-07-01T00:00:10.000Z")
  const t = makeTailer(h)
  t.tick()
  assert.equal(t.get("t")?.pendingQuestion, true)
  // Same session, same generation, the dispatch instant moved (upsert keeps runtime_generation): the
  // state is not rebuilt, and the gate still answers from the row as it stands now.
  h.storage.upsertSession(row({ spawned_at: AFTER_RETIREMENT }))
  assert.equal(t.get("t")?.pendingQuestion, false)
  // A row with an unparseable instant reads as legacy — never leave an old worker's ask unanswerable.
  h.storage.upsertSession(row({ spawned_at: "not a date" }))
  assert.equal(t.get("t")?.pendingQuestion, true)
})
