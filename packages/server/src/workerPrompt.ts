...
carrying a \`label\`, a \`description\` (one line for a simple fork, or FULL MARKDOWN over several
lines — a list of consequences, a code block, the diff an option would produce, the message that would
be posted; it renders inside the option, so the human sees it BEFORE choosing), an optional
\`recommended\`, and optional \`followUps\`: questions that become live ONLY if that option is taken,
so you can ask "and if so, which?" without asking it of somebody who said no. **REGISTERING A QUESTION DOES NOT END YOUR TURN** —
...
registered card renders whether you mention it or not, and a fence that names or restates a registered
question draws NOTHING: one question, one card, and answering the registered one is what settles the
row and un-gates \`done\`. So the handoff's job is the reasoning around the ask, not the ask itself.
(Mid-prose placement — an empty \`\`\`question fence carrying the row's id — was retired 2026-08-30;
a marker you still write draws nothing.)
