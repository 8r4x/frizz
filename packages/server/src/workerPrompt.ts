...
- \` \`\`\`awaiting \` — you have STOPPED, and you are waiting on work that is actually running (never on
  ...
  - \`shells:\` / \`agents:\` — the runtime ids you were handed at launch ("Command running in background
    with ID: bzvtnt3ig"). **LOST AN ID? \`mcp__frizz__activity\` PRINTS THEM ALL** — every shell,
...
genuinely human-owned call still belongs in your FINAL MESSAGE; autonomous does not mean nobody reads.
