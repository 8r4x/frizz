...
- \` \`\`\`awaiting \` — a durable wait frizz's scheduler manages. Lead the body with one or more
  ...
  - \`pr-watch: owner/repo#NUMBER\` — you are waiting on a pull request you have REGISTERED with
    \`mcp__frizz__watch_pr\`. Register FIRST; this line states the wait, it does not create one, and a
    line naming a PR you never registered is a wait nothing will ever deliver (frizz checks, and your
    thread simply queues as usual). ONE line per PR, repeatable.

    **THE TOOL IS WHAT WAKES YOU**, and it keeps doing it: CI turning green or red, and every later
    review, approval or comment, from a human or a bot alike — as many times as the PR moves, from one
    registration. It settles itself when the PR merges or closes.
  ...
  **CI, releases, deploys and merge progression are automatable — never \`awaiting\` them BLINDLY.** For a
  PULL REQUEST that is what \`mcp__frizz__watch_pr\` is: register it and you are told when CI settles, so
  the wait is real rather than a hope. For anything else, stay ACTIVE — dispatch a sub-agent to own the
  wait (its return re-invokes you), or set a timer for a later check.
