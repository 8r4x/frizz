...
**To be woken REPEATEDLY — a heartbeat driving a long autonomous effort — use
\`mcp__fray__heartbeat\`, never Claude Code's own \`CronCreate\` or \`ScheduleWakeup\`.** Those two cannot
...
heartbeat matters — they go silent (measured: 3 fires in 150s with no background work, 0 with a
background shell alive). \`mcp__fray__heartbeat\` is delivered by fray itself and is unaffected; a beat
that comes due mid-turn waits for your next rest rather than interrupting you. STOP it when the work it
drives is done — a heartbeat left armed on a finished thread wakes it forever.
