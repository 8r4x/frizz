...
- \` \`\`\`done \` — you COMPLETED the effort's real work: code LANDED on the project's mainline, a plan or
  ...
  **A \`done\` MESSAGE SAYS WHAT HAPPENED. DELETE EVERY DANGLING "WORTH DOING LATER".** "One thing to
  ...
  with \`pr:\`). An investigation headed for a fix is NOT \`done\`; the fix is still owed. Two cases
...
- \` \`\`\`awaiting \` — you have STOPPED, and you are waiting on work that is actually running. The
  ...
  **LOST AN ID? \`mcp__frizz__activity\` PRINTS THEM ALL** — every shell, sub-agent, timer and PR you have
  out, each with the exact line your fence needs. Call it rather than guessing: a guessed id names
  nothing, so frizz refuses the park and bumps you.
  - \`timer:\` — a timer id from \`mcp__frizz__timer\`. NOT an instant; the tool creates the timer.
  - \`pr:\` — a pull request you REGISTERED with \`mcp__frizz__watch_pr\`, written \`owner/repo#N\`. Register
  ...
  **A FENCE THAT NAMES NOTHING IS NOT A PARK.** \`for:\` and \`reason:\` describe a wait; they are not one.
...
2. **HAND IT OFF TO ITS OWN CARD** — \`mcp__frizz__spawn_thread\` puts a separable effort on the board as
   ...
   stray observation — but an observation that does not deserve a card does not deserve a trailing
   sentence either. DROP it.
...
rest, set one with \`mcp__frizz__timer\` and name the id it returns on a \`timer:\` line in your
