// The standing prompt's two SERVER-side invariants (scheduler.ts SOURCE 5), each of which is a way the
...
// alone once it answers ALLDONE lives in backend/_live_standing_prompt.mts — a live probe, not a unit
...
// be a thread whose standing prompt can never be closed.
...
function fixture() {
  const dir = mkdtempSync(join(tmpdir(), "fray-standing-"))
  const storage = createStorage(join(dir, "ui.db"))
  const slug = "standing-t"
...
test("storage: toggling off and on KEEPS the generation and the last-fired stamp", () => {
  const f = fixture()
  try {
    assert.equal(f.storage.setStandingPromptIfCurrent(f.slug, "sid", 0, "keep going", true, "2026-08-02T00:00:00.000Z"), true)
    const armedAt = f.row().standing_prompt_armed_at
    assert.equal(armedAt, "2026-08-02T00:00:00.000Z")
    f.storage.stampStandingPromptFired(f.slug, armedAt!, "2026-08-02T00:05:00.000Z")

    f.storage.setStandingPromptIfCurrent(f.slug, "sid", 0, "keep going", false, "2026-08-02T00:10:00.000Z")
    assert.equal(f.row().standing_prompt_enabled, 0)
    assert.equal(f.row().standing_prompt_armed_at, armedAt, "an off/on flip is not a re-arming")
    f.storage.setStandingPromptIfCurrent(f.slug, "sid", 0, "keep going", true, "2026-08-02T00:11:00.000Z")
    assert.equal(f.row().standing_prompt_enabled, 1)
    assert.equal(f.row().standing_prompt_armed_at, armedAt)
    // The rate floor survives the flip too — otherwise toggling would be a way to bypass it.
    assert.equal(f.row().standing_prompt_last_fired_at, "2026-08-02T00:05:00.000Z")
  } finally {
    f.close()
  }
})
...
test("storage: EDITING the text mints a new generation and drops the last-fired stamp", () => {
  const f = fixture()
  try {
    f.storage.setStandingPromptIfCurrent(f.slug, "sid", 0, "keep going", true, "2026-08-02T00:00:00.000Z")
    f.storage.stampStandingPromptFired(f.slug, f.row().standing_prompt_armed_at!, "2026-08-02T00:05:00.000Z")
    f.storage.setStandingPromptIfCurrent(f.slug, "sid", 0, "do something else", true, "2026-08-02T00:10:00.000Z")
    assert.equal(f.row().standing_prompt_armed_at, "2026-08-02T00:10:00.000Z", "new words are a new generation")
    assert.equal(f.row().standing_prompt_last_fired_at, null, "and the new words have never fired")
  } finally {
    f.close()
  }
})
...
test("storage: a null prompt clears the whole row, and a stale session/generation writes nothing", () => {
  ...
  try {
    f.storage.setStandingPromptIfCurrent(f.slug, "sid", 0, "keep going", true, "2026-08-02T00:00:00.000Z")
    assert.equal(
      f.storage.setStandingPromptIfCurrent(f.slug, "other-sid", 0, "hijack", true, "2026-08-02T00:01:00.000Z"),
      false,
      "a tab looking at a superseded session fails closed",
    )
    assert.equal(f.row().standing_prompt, "keep going")
    f.storage.setStandingPromptIfCurrent(f.slug, "sid", 0, null, true, "2026-08-02T00:02:00.000Z")
    assert.equal(f.row().standing_prompt, null)
    assert.equal(f.row().standing_prompt_armed_at, null)
    assert.equal(f.row().standing_prompt_enabled, 0, "a cleared row can never read as enabled")
  } finally {
    f.close()
  }
})
