// LIVE PROBE: for a codex background exec that frizz ACTUALLY RENDERS as a shell row, can frizz name the
...
// frizz can ADDRESS one — and running frizz's own `readCodexTranscriptFile` over that probe's rollout
// showed why the question is live: the two `Exec` calls it projected were both `completed`, with no
// `backgroundState` at all. That exec never went through `yield_control()`, so frizz would not have
// drawn a shell row for it in the first place, and "we can kill it" was about a row that does not exist.
//
// So this probe insists on the shape frizz actually renders — `codexExplicitBackground()`, an exec whose
// script calls `yield_control()` — and then asks the only question left:
//
//   Q1. Does frizz project it as a LIVE BACKGROUND shell row (status pending + backgroundState
...
//       processId? That is the fallback supply route, and frizz already receives those and discards
...
try {
  ...
  // time, which frizz does not classify as background. `yield_control()` is the call frizz keys on, so
  ...
  // ---- What FRIZZ sees, through its own reader ---------------------------------------------------
  ...
  console.log("[probe] every exec-ish call frizz projected:")
  ...
  ok("Q1 frizz projects it as a LIVE BACKGROUND shell row", liveBackground.length > 0,
  ...
  console.log(`[probe] handles frizz projected: ${JSON.stringify(handles)}   processId the app-server wants: ${JSON.stringify(terminal?.processId)}`)
  ok("Q2 a projected handle EQUALS the app-server's processId", handles.includes(String(terminal?.processId)),
    handles.includes(String(terminal?.processId)) ? "the row can address its own kill" : "frizz must get the id from the stream instead")
  ...
  ok("Q3 processId is on the item/* stream frizz already receives", streamed.length > 0)
