...
// worker inherits the operator's environment and is denied only fray's own control plane. Run:
...
// half. The half that only a real dispatch can show is that the value survives every gate between fray
...
//   · FRAY_* MUST NOT — a worker dispatched to work on fray would otherwise read the broker's daemon
...
// Set in FRAY's process, never passed through workerEnv — inheritance is the only way these can arrive.
const INHERITED_VALUE = "inherited-from-the-operator-shell"
process.env.FRAY_LIVE_PROBE_TOOLCHAIN = "must-not-cross" // fray-prefixed on purpose: the denied case
...
try {
  ...
  const { slug, sessionId } = await ctx.dispatcher.dispatch({
    prompt: `Run exactly this one Bash command and then stop: ` +
      `printf '%s|%s|%s' "\${LIVE_PROBE_TOOLCHAIN:-MISSING}" "\${FRAY_LIVE_PROBE_TOOLCHAIN:-DENIED}" "\${FRAY_CLAUDE_BROKER:-DENIED}" > ${reportFile}`,
  }, { backend: "claude" })
  ...
  const [inherited, frayProbe, frayBroker] = raw.split("|")
  ok("an ordinary operator variable REACHES the worker", inherited === INHERITED_VALUE, `worker read ${JSON.stringify(inherited)}`)
  ok("a FRAY_* variable does NOT reach the worker", frayProbe === "DENIED", `worker read ${JSON.stringify(frayProbe)}`)
  ok("the broker daemon payload does NOT reach the worker", frayBroker === "DENIED", `worker read ${JSON.stringify(frayBroker)?.slice(0, 80)}`)
