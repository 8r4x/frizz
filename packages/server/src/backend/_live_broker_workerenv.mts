...
try {
  ...
  const { slug, sessionId } = await ctx.dispatcher.dispatch({
    prompt:
      ...
      `1. Use the Agent tool with subagent_type exactly "fray:haiku" to dispatch a sub-agent whose prompt is: ` +
      `"Use the Write tool to create the file ${subFile} containing the word DONE, then stop." ` +
      `If subagent_type "fray:haiku" is unavailable, instead write the file ${reportFile} containing "NO_FRAY_AGENT".\n` +
      `2. Write the file ${reportFile} listing whether you can see tools named mcp__fray__spawn_thread and mcp__chrome-devtools (one per line, "yes" or "no").`,
  }, { backend: "claude" })
  ...
  // Wait for the sub-agent's marker file (fray:haiku ran) or the report file (worker finished/failed).
  const deadline = Date.now() + 180_000
  while (!existsSync(subFile) && Date.now() < deadline) await sleep(1_000)

  ok("broker worker dispatched a fray:haiku SUB-AGENT that ran (SUB_OK.txt written)", existsSync(subFile),
