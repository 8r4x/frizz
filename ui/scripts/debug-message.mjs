...
//   raw right + projected WRONG → the projector mis-parsed        → ui/packages/server/src/transcript.ts
//   raw right + projected right → the renderer mis-drew a correct projection → web/src/components/ChatView.tsx
//
//   node ui/scripts/debug-message.mjs claude:8f2c…:24009
//   node ui/scripts/debug-message.mjs codex:01ab…:9312:2 --raw   # raw records only
//   node ui/scripts/debug-message.mjs claude:8f2c…:24009 --json  # machine-readable, for piping
//
...
async function main(argv) {
  ...
  const id = args[0]
  if (!id) die("usage: node ui/scripts/debug-message.mjs <sourceId> [--raw|--projected|--json]")
