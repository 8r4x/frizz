...
const home = opt("home")
const cwd = opt("project", process.cwd().replace(/\/ui$/, ""))
...
execFileSync("sqlite3", [db, `INSERT OR REPLACE INTO session
  ...
  VALUES ('${SLUG}', '${SESSION_ID}', '${tmuxName}', '${now}', 'Md task render', 'claude', 'opus', 'high', 'default', 0, 0, 0, 0, 0, 0);`])

const api = createRpcClient(`http://127.0.0.1:${port}/`)
await api.waitForHealth()
console.log(JSON.stringify({ url: `http://127.0.0.1:${port}/thread/${SLUG}`, slug: SLUG, tmuxName, socket }))
