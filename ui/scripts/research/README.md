...
Evidence for whether fray's hand-rolled Codex daemon (`backend/codex-app-server-daemon.ts` +
...
rejected (`-32600 Already initialized`). fray's daemon caches and replays the handshake because it
...
**Events are DROPPED, not queued, while nobody is attached — but the two things fray's queue exists to
...
- A turn that completed ENTIRELY while detached replays nothing. But `thread/resume` reports
  ...
  than fray's queue, which silently overflows past `MAX_QUEUED_LINES`/`MAX_QUEUED_BYTES`.
