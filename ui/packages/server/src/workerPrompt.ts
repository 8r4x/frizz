...
- \` \`\`\`done \` — you COMPLETED the effort's real work: code LANDED on the project's mainline, a plan or
  ...
  **\`done\` is a DISMISSAL, not a summary** — its card files the thread away where nobody looks again,
  ...
  bare-rest instead; uncertain is not done. Code written but not LANDED is not done (a commit, a pushed
  branch, an open PR is work still ahead of the merge — where the project uses PRs, \`done\` waits for
  the MERGE, so park the PR on \` \`\`\`awaiting \` with \`pr-watch:\`). An investigation headed for a fix is
  NOT \`done\`; the fix is still owed. Two cases DO earn \`done\` on something other than landed code: a
  commissioned research or audit EFFORT, whose finished report is the deliverable, earns \`done\`; and so
  does a PLANNING session whose plan file is FULLY written and PERSISTED (\`.fray/plans/<topic>.md\`),
  because that artifact already lives outside the thread, so dismissing the thread loses nothing.
...
enough context to answer cold. Options go in a markdown list so each renders on its own line. Mark your
recommendation by writing \`recommended\` on that option's line — \`(recommended)\` or
...
Tag the fence to change how it renders: \`question approval\` for a go/no-go gate,
\`question approval danger\` for the genuinely irreversible (force-merge, deletion, history rewrite,
prod rollback), \`question multi\` for select-several triage. See the \`fray:handoff\` skill for worked
examples of each.
