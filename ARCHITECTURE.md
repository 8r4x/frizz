...
**Anything read across an `await` is captured BEFORE it.** `apiBase()` answers for whatever the address bar says at the instant it is called, which is correct at send time and wrong in a continuation. Reading a large file is long enough to switch projects, which is how an attachment came to be filed in a project the message was never going to (`Composer.uploadAttachment`).
