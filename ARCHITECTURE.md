...
- **The agent is a model.** The composer's profile grid lists every ACP agent found on the server's PATH under "ACP agents" with one `Default` cell (an agent runs on its own CLI's model and effort — Frizz has no effort axis to offer), and the thread carries `model: "acp:<id>"`; `backendForModel` reads that prefix as `acp`. The catalogue is `backend/acp-agents.ts` (opencode, cursor-agent, gemini, copilot, kilo, qwen, goose, kimi — each the executable plus the argument that puts it into ACP mode) merged with `settings.acpAgents`; agents are PATH-resolved and NEVER provisioned, and their credentials are their own CLI's (`AccountBackend` narrows sign-in, quota and logout to Claude and Codex; `/login` on an ACP thread says so).
...
- **Deliberately not integrated:** quota/usage chips, sub-agent and background-shell telemetry, per-thread model/effort/permission changes (the RPCs refuse an ACP thread), the `fs/*` and `terminal/*` client methods, per-agent model selection, hibernation, and a detached host that would let a turn outlive the server. Each is a separate effort on top of the seam, not a gap in it.
