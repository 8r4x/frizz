...
| `bash-background.mjs` | PreToolUse(Bash) | new | Denies a local shell job that escapes through `&` without a later `wait`/`kill`/trap. Shell job control bypasses Claude's task registry, so Fray cannot track or wake it; self-contained probe concurrency remains allowed. |
