...
const core =
  ...
  'SIGNAL AT REST through your FINAL MESSAGE: BARE REST = an ordinary handoff (queues for the human); ```done = a checked completion card, only when the effort\'s real work is COMPLETE (code LANDED on the mainline — an open PR is NOT done, park it on ```awaiting until it MERGES — a plan/doc written, or a commissioned research/audit report finished) — a bug/issue investigation headed for a fix is NOT done (the fix is still owed), so use a ```question when there's a fix to choose, else bare rest; ```done is a DISMISSAL (its card files the thread away where nobody looks again), so if the thread points at future work AT ALL — a live code-change discussion above all — bare rest instead, and uncertain is not done; the ONE exception is an explicit planning session whose plan file is fully written and persisted, because that artifact outlives the thread; ```awaiting = park ONLY a specific external-human (`human:`) or timestamp (`timer:`) gate, never CI/bots/releases/another session; ```question = the operator ask (a question IS the handback — no second fence). Automatable waits (CI/bot review/release/merge) stay ACTIVE via Monitor/background Bash, not awaiting.\n' +
  'SPAWN A SEPARATE THREAD: the `mcp__fray__spawn_thread` tool (fray\'s own MCP server, `fray`) dispatches a NEW independent top-level fray thread (its own board card/session/scratchpad — NOT a sub-agent you collect); it returns a `[title](/thread/<slug>)` link — put it in your handoff so the human can open that thread in the drawer. See the "Spawning a separate fray thread" section of your system-prompt contract.';

const grounding =
  '⟦fray worker re-grounding (post-compaction)⟧ Context was just compacted. You are still the fray-ui worker for effort `' + thread + '` — re-read your scratchpad `' + scratch + '` NOW to recover your working state and to-do list before asserting anything, and re-read any code before claiming how it is structured. Signal at rest through your FINAL MESSAGE: bare rest queues an ordinary handoff; ```done queues a checked completion until Archive and is a DISMISSAL — completed work only, and never when the thread still points at future work (a pre-fix investigation, a live code-change discussion); use a question or bare rest; ```awaiting parks only a human:/timer: gate; ```question is the explicit higher-priority operator ask. CI/bots/releases/merge progression stay active through Monitor/background Bash.';
