...
const grounding =
  '⟦fray worker re-grounding (post-compaction)⟧ Context was just compacted. You are still the fray-ui worker for effort `' + thread + '` — re-read your scratchpad `' + scratch + '` NOW to recover your working state and to-do list before asserting anything, and re-read any code before claiming how it is structured. Signal at rest through your FINAL MESSAGE: bare rest queues an ordinary handoff; ```done queues a checked completion until Archive (completed work only — a pre-fix investigation isn't done, and never done while a code-change discussion is live; use a question or bare rest); ```awaiting parks only a human:/timer: gate; ```question is the explicit higher-priority operator ask. CI/bots/releases/merge progression stay active through Monitor/background Bash.';
