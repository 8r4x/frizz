...
2. Pass both fields explicitly on every Fray dispatch, using an exact model slug advertised by the active tool. For an independent child, also set the schema's context-fork control to no parent history, under whatever name it exposes — current Codex names it `fork_turns` (pass `"none"`); older builds used `fork_context: false`. The default may be a FULL history fork, so set it explicitly when independence matters; never invent a field the live schema lacks (a missing or unfamiliar context-fork control is not by itself a routing failure — keep the child self-contained and say so).
