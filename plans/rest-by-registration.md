...
- **A live `awaiting` fence beside an open question draws as prose** (2026-08-28). The resting card already yielded to an open question (`deriveAwaitingBackground`), but a worker's own fence card in the message did not, so a parked-looking card — hourglass, shell table — stacked above the ask on a thread that had registered a question, kept working, and fenced its next rest (maintainer: "Weird that there's both an awaiting block and open questions"). The fence's body is the handoff and stays, as prose; the chrome goes, the question is the one card, and the monitors stay on the strip under the prompt box — the fence path now renders as the registration path does. The contract says not to write the fence there at all.
