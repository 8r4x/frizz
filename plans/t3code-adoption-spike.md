...
[pingdotgg/t3code](https://github.com/pingdotgg/t3code) into frizz, without regressing frizz's DX.

## Provenance

The investigation behind this document ran as a **non-frizz** Claude Code session
...
to that clone; all frizz paths are relative to this repo root.
...
`apps/server/src/provider/Layers/EventNdjsonLogger.ts:31`. On the frizz side: `persistSession: true` at
...
**Answer: frizz's broker daemon is better than t3code's model on the axis that matters, and it should
be kept.**
...
are foreground and interactive. It is *not* fine for frizz, whose threads are unattended multi-hour
dispatches and where a frizz restart during a dev cycle is routine.

Frizz's broker already solves this, and its own header says so
(`packages/server/src/backend/claude-agent-broker.ts`): *"the session OUTLIVES frizz, so frizz
...
survival, which t3code gave up. Closing frizz without terminating sessions is not too difficult — it is
...
> forensics to disk (`backend/claude-broker-diagnostics.ts`), because a relay only reaches a frizz that
> is attached, which a frizz crashing or restarting is not.
>
...
— when frizz is gone there is no client, and the event is dropped. There is no backlog, no replay
cursor, no "events since". The comment at `:132` states the intent plainly: *"frizz gone; the session
...
transcript to JSONL, plus the tailer re-reading that file when frizz comes back. In other words: the
recovery mechanism is the same write-to-disk-and-re-read that t3code uses. The daemon's marginal value
over t3code is narrower than it first appears — it is *"the agent keeps making progress while frizz is
...
event backlog in the daemon plus a resume-from-sequence handshake, so a reconnecting frizz replays what
it missed. Do that **before** removing any tailer responsibility, and verify it by killing frizz
...
model reroutes. Frizz's `NormalizedEvent` (`packages/server/src/backend/types.ts:42`) has 10 kinds
...
`streamEvents` / `capabilities`. Frizz's `AgentBackend` is `buildSpawn(argv)` / `transcriptPath` /
...
no LLM, no browser. Frizz's only gate for orchestration logic today is "launch a promoted artifact,
...
revert-to-turn-N — frizz has none of it.

Side benefit: the temp-`GIT_INDEX_FILE` trick is exactly the fix for frizz's known shared-index commit
...
Personal" / "Claude OpenRouter" as separate providers. Frizz is single-account by construction, but
...
`main` upstream and publishes both `schema/json/` and `schema/typescript/`. Frizz hand-wrote 3,490 lines
...
- **Repo-local lint plugin for house invariants.** `oxlint-plugin-t3code` — e.g.
  `no-global-process-runtime` bans `process.platform`/`process.arch` outside one file. Frizz encodes its
...
- **In-app preview + project scripts.** `apps/server/src/preview/Manager.ts` — tabbed browser preview
  sessions per thread, port scanning, `ProjectScript` entries for dev/test commands. Relevant to frizz's
...
1. **`AskUserQuestion` intercepted at `canUseTool` → a structured question channel.**
   ...
   it empty and denies. This is the direct replacement for frizz's ` ```question ` fence convention,
   which exists only because the tmux era had no channel. Frizz's `ARCHITECTURE.md` records two prior
   designs (blocking MCP tool; `frizz-ask` CLI + sidecars) built and rejected as fragile — `canUseTool`
...
2. **`ExitPlanMode` intercepted, denied, and captured.** Same file, `:3347`. The plan markdown is
   ...
   a later turn." Maps onto frizz's Plans section.
...
5. **`settingSources: ["user","project","local"]`.** They deliberately load the user's own CLAUDE.md and
   settings rather than running a hermetic agent. Frizz should make this an explicit posture either way.
...
8. **Sub-agents and hooks as first-class canonical events.** `task_started` / `task_progress` /
   ...
   (`ClaudeAdapter.ts:2642-2727`). Frizz reconstructs sub-agents with two bespoke trackers for what the
...
9. **Runtime mode as a two-value switch mapped per provider.** `docs/architecture/runtime-modes.md`:
   ...
   + `workspace-write`. One user-facing concept, N provider mappings — versus frizz's `PermissionMode`
...
- **Full event sourcing with SQL projections.** Frizz's `.frizz/` markdown-as-source-of-truth is
  deliberate — the files are user-editable and `frizz-update` is a real CLI surface. Take the *shape*
  (validated commands → events → projections) only where frizz already owns a DB: `ui.db`.
- **Relay / Tailscale / SSH / mobile / cloud auth.** That is a product decision about whether frizz stops
  being localhost-and-one-repo, not an architectural improvement.
- **The weight.** 523k LOC across 5 apps with a team, vs frizz's 117k. Copy mechanisms, not scale.
- **Deleting the broker.** See the headline section. t3code's lack of one is a limitation it accepts,
  not a design frizz should adopt.
