# Where Frizz's global state lives

Spec, 2026-08-01. `~/.frizz` is not idiomatic; honor XDG where it is defined, write to the right place
on Windows, and fall back to `~/.frizz`. Downstream of [`gitless-projects.md`](gitless-projects.md)
...
1. `~/.frizz` — the legacy location, and the one an upgrading user's live state is actually in.
2. `$XDG_DATA_HOME/frizz`, when that variable is set.
3. `%LOCALAPPDATA%\Frizz` on Windows.
4. The OS-native default for the platform (`~/.local/share/frizz`, and see §macOS).
...
Frizz create one, and then it uses the most idiomatic place available:

1. **The XDG variables, each honored individually when SET** — `$XDG_DATA_HOME`, `$XDG_STATE_HOME`,
   `$XDG_CACHE_HOME`, `$XDG_RUNTIME_DIR`, each `+ /frizz`. A user who defines only `XDG_CACHE_HOME`
   gets the cache relocated and nothing else.
2. **`%LOCALAPPDATA%\Frizz` on Windows** — see below. Windows never falls through to a dotfile.
3. **`~/.frizz`** otherwise, with today's layout unchanged.

So the only user who ever sees a new path is one who has never run Frizz, which is exactly the
...
| --- | --- | --- | --- |
| `builds/` — promoted artifacts | `~/.frizz/builds` | **1.8 GB** | **cache** — regenerable from source, deduped by digest |
| `projects/<id>/browser-profile` | `~/.frizz/projects` | **1.51 GB** | **cache** — a Chrome profile, rebuilt on demand |
| `quota-cache/` | `~/.frizz/quota-cache` | 4 KB | **cache** — it says so |
| `projects/<id>/ui.db`, attachments, identity | `~/.frizz/projects` | **27.8 MB total** | **data** — the threads. Losing this loses the product |
| logs, `launcher.json`, locks, owner files | `~/.frizz/projects` | small | **state** |
...
~/.cache/frizz` frees 3.3 GB and costs nothing but a rebuild, backup tools skip it by default, and the
27.8 MB that actually matters sits in `~/.local/share/frizz` where a backup will find it. Under
`~/.frizz` those three populations are indistinguishable, which is why the directory grows to gigabytes
and nobody dares delete it.

## Mapping

```
data    $XDG_DATA_HOME/frizz      ~/.local/share/frizz    projects/<id>/{ui.db,attachments,identity.json}
state   $XDG_STATE_HOME/frizz     ~/.local/state/frizz    projects/<id>/{logs,launcher.json,*.lock,*.owner}
cache   $XDG_CACHE_HOME/frizz     ~/.cache/frizz          builds/, projects/<id>/browser-profile, quota-cache/
runtime $XDG_RUNTIME_DIR/frizz    $TMPDIR                sockets, port reservations
```
...
```
data     %LOCALAPPDATA%\Frizz\Data
state    %LOCALAPPDATA%\Frizz\State        (or \Logs for the log subtree, matching convention)
cache    %LOCALAPPDATA%\Frizz\Cache
...
`%USERPROFILE%\AppData\Local\Frizz`, and only then to `~/.frizz`. There is precedent for reading it:
`src/browser.ts:131` already resolves Chrome through `LOCALAPPDATA`.

### macOS

The strictly idiomatic answer is `~/Library/Application Support/Frizz` and `~/Library/Caches/Frizz`.
This spec deliberately does **not** use it: the rule is XDG-if-set, then `~/.frizz`, and that matches
what every tool in this product's neighbourhood does — `~/.claude`, `~/.codex`, `~/.gitconfig`. A
macOS user who has set `XDG_*` (many developers do) gets XDG; everyone else keeps `~/.frizz`.
...
**An existing `~/.frizz` keeps winning, permanently.** Not a deprecation window, not a nag.
...
it themselves while Frizz is stopped — `mv ~/.frizz ~/.local/share/frizz` and so on — and rule 1 stops
applying the moment `~/.frizz` is gone. Document that in the FAQ; do not automate it.
...
`~/.frizz` is hardcoded at **64 sites across ~14 source files** (plus ~28 dev scripts) — `project.ts`,
...
1. One module — `frizzPaths({ env, platform, home })` returning `{ data, state, cache, runtime }`,
...
2. Replace the call sites mechanically. Most read `join(home, ".frizz", "projects", id)` and become
...
4. Dev scripts can keep hardcoding `~/.frizz` until the module exists, then import it.
...
- **Linux with `XDG_DATA_HOME` unset.** Strict XDG says use the `~/.local/share` default
  ...
  on `~/.frizz`. That is the conservative reading of "XDG if it is defined" and it means Linux mostly
