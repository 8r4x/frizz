...
**A pending question does not stop a rest, and `mcp__frizz__ask` IS the sign-off.** Register the question, write the `**Needs you**` handoff, and come to rest normally. Frizz draws every open question at that rest whether the write-up mentions it or not, so what the write-up chooses is WHERE: put the row's id in an EMPTY question fence (an opening ` ```question qst_ab12cd34 ` line, a closing ` ``` ` line, nothing between) at the point the ask belongs, and the registered card is drawn there rather than at the end. A fence that restates the question in its body is matched on the text instead. Place every open question, or `unask` the ones that no longer matter.
