// @ts-check
/**
 * frizz — per-thread SESSION OWNERSHIP tests. Run with `node --test 'board/*.test.mjs'`.
 *
 ...
 *   2. The `frizz claim / disown / owners` CLI — the claim policy: EASY on unowned/orphaned,
...
function frizz(dir, args, sid = SID_A) {
  try {
...
function fixture() {
  const dir = mkdtempSync(join(tmpdir(), 'frizz-own-'));
  mkdirSync(join(dir, '.frizz', '.session-state'), { recursive: true });
  return dir;
}
...
test('setOwner/readOwner: round-trip, removal, and byte-preservation of the rest', () => {
  ...
  try {
    ...
    assert.equal(readFileSync(join(dir, '.frizz', 't.md'), 'utf8'), body, 'clearing restores the exact original bytes');
  } finally {
...
test('frizz claim: EASY on unowned; already-yours is a no-op', () => {
  const dir = fixture();
  try {
    thread(dir, 'task');
    let r = frizz(dir, ['claim', 'task'], SID_A);
    assert.equal(r.status, 0);
    assert.match(r.stdout, /CLAIMED task.*was unowned/);
    assert.equal(readOwner(dir, 'task'), SID_A);
    r = frizz(dir, ['claim', 'task'], SID_A);
...
test('frizz claim: a DIFFERENT LIVE owner is REFUSED without --force, taken WITH --force', () => {
  const dir = fixture();
  try {
    ...
    r = frizz(dir, ['claim', 'task', '--force'], SID_A);
    assert.equal(r.status, 0);
...
test('frizz claim: an ORPHANED thread (dead owner) is EASILY claimable, no --force', () => {
  const dir = fixture();
  try {
    thread(dir, 'task', `owner_session: ${SID_B}\n`); // B owns but never heartbeats → dead
    const r = frizz(dir, ['claim', 'task'], SID_A);
    assert.equal(r.status, 0);
...
test('frizz owners --gc clears orphaned owners but leaves live + mine', () => {
  const dir = fixture();
  try {
    ...
    const r = frizz(dir, ['owners', '--gc'], SID_A);
    assert.equal(r.status, 0);
